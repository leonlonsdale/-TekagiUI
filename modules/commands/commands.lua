...
TekagiUI.Modules.Commands = {}

local M = TekagiUI.Modules.Commands
...
for key, data in pairs(CommandMap) do
    ...
    local function ToggleCommand(enabled)
        local globalName = "TEKAGI_" .. key:upper()
        if enabled then
    ...
    M.settings.settings[key] = {
        label = data.label,
        heading = isFirst and "Commands" or nil,
...
frame:SetScript("OnEvent", function()
    if TekagiUIDB and TekagiUIDB.Commands then
        for key, _ in pairs(CommandMap) do
            local userSetting = TekagiUIDB.Commands[key]
            local isEnabled = userSetting
            if userSetting == nil then isEnabled = M.defaults[key] end
            
            M.settings.settings[key].callback(isEnabled)
        end
    end
end)