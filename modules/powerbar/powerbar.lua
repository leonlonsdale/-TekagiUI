...
local function UpdateSegments(current, max)
	...
	for i = 1, max do
		if not segments[i] then
			...
			segments[i].tex:SetPoint("TOPLEFT", 1, -1)
			segments[i].tex:SetPoint("BOTTOMRIGHT", -1, 1)
...
eventFrame:SetScript("OnEvent", function(self, event)
	if event == "PLAYER_ENTERING_WORLD" then
		...
		TekagiUIDB.PowerBar = TekagiUIDB.PowerBar or { enabled = true, height = 10 }
	...
	if class == "WARLOCK" then
		sType = Enum.PowerType.SoulShards
	elseif class == "MONK" then
		sType = Enum.PowerType.Chi
	end
...
if _G["EssentialCooldownViewer"] then
	_G["EssentialCooldownViewer"]:HookScript("OnSizeChanged", AnchorBars)
end
